...
int wifi_request_count = 0; // Initializing a counter
...
void runServo() {
    ...
    if (currentMillis_servo - previousMillis_servo >= interval_servo) {
        ...
        // Sweep servo based on direction flag
        if (increasing) {
            pos++;
            if (pos >= 120) {
                increasing = false;
...
#define LOX1_ADDRESS 0x38
#define LOX2_ADDRESS 0x39
#define LOX3_ADDRESS 0x40
...
void read_tof() {
  ...
  if(measure1.RangeStatus != 4) {     // if not out of range
    ...
    if (measure1.RangeMilliMeter < 100) {
        Serial.println("Obstacle detected! Adjusting servo...");
        runServo();  
    }
  } else {
...
bool is_alive() {
    uint8_t health = receive_I2C_byte();  // Get the health status
    ...
    if (health == 0) {
        ...
        for (int i = 0; i < 4; i++) {
            ledcWrite(motorPINEN[i], 0); // Stop all motors
            motorVelSetpoint[i] = 0.0;
        }
        return false; // Robot is not alive
    }
    return true; // Robot is alive
}
...
// ################################################################################################
  ...
  servo.attach(SERVO_PIN, 1000, 2000); // Attach servo to pin 42 with min/max pulse widths
...
void handleRoot() {
  ...
  wifi_request_count++;
  Serial.print("Number of Wi-Fi requests handled: ");
  Serial.println(wifi_request_count);
...
void loop() {
  ...
  //if (!is_alive()) {
        // If health is zero, we bypass the loop
      //  return;
    //}
